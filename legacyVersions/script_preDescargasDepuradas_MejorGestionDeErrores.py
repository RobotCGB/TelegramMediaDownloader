...
# TODO: añadir opción de mover descargas a carpeta concreta
    try:
            ...
            path_complete = os.path.join(complete_folder, file_name)
            ...
            _, extension = os.path.splitext(path_real)

            if os.path.splitext(file_name)[1].lower() == extension.lower():
                final_name = file_name
            else:
                final_name = f"{file_name}{extension}"

            # Construir destino con extensión
            path_complete = os.path.join(complete_folder, final_name)
